...
class OrderItemSerializer(serializers.ModelSerializer):
    selected_options = OrderItemSelectedOptionSerializer(
...
class CreateSelectedOptionSerializer(serializers.Serializer):
    ...
    extra_name_fi = serializers.CharField(
        max_length=100, required=False, allow_blank=True, default="")
    option_name = serializers.CharField(max_length=100)
    option_name_fi = serializers.CharField(
        max_length=100, required=False, allow_blank=True, default="")
...
class CreateOrderItemSerializer(serializers.Serializer):
    ...
    menu_item_name_fi = serializers.CharField(
    ...
    selected_options = CreateSelectedOptionSerializer(
...
class CreateOrderSerializer(serializers.Serializer):
    ...
    payment_method = serializers.ChoiceField(
        choices=["online", "cash_on_delivery", "cash_on_pickup"],
        default="online"
    )
    ...
    def create(self, validated_data):
        ...
        for item_data in items_data:
            ...
            for opt in options_data:
                OrderItemSelectedOption.objects.create(
                    order_item=order_item, **opt)
        ...
        try:
            send_order_received_email.delay(
                order_id=order.id,
                user_email=order.guest_email or (
                    customer.email if customer else ""),
                user_name=order.guest_name or (
                    customer.get_full_name() if customer else "Guest"),
