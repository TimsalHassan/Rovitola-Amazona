      ...
      <div className="max-w-[1200px] mx-auto px-4 py-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <div className="flex flex-col gap-2 text-sm mt-4 pt-4 border-t border-white/5">
              <Link
                to="/privacy"
        ...
        <div className="border-t border-white/5 mt-8 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-gray-500">
          <p>
            © {new Date().getFullYear()} {info?.name ?? "Ravintola Amazona"}.{" "}
            {t("footer.rights")}
          </p>
          {info?.is_delivery_enabled && (
            <p className="text-amber-500/70">
              {t("footer.deliveryFrom")} €{info.min_order} ·{" "}
              {t("footer.deliveryFee")} €{info.delivery_fee}
            </p>
          )}
        </div>
      </div>
