...
import { ADMIN, adminGet, adminHeaders } from "../../api/admin";
...
function StatCard({
  ...
  label: string; value: string | number; sub: string; icon: string; accent?: boolean; to?: string;
}) {
  const inner = (
    <div className={`relative rounded-2xl p-4 overflow-hidden border transition-all ${accent ? "bg-amber-500 border-amber-400" : "bg-[#0a0f1e] border-white/5 hover:border-white/10"}`}>
...
export default function AdminDashboardPage() {
    ...
    return (
      <div className="space-y-5">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-[#0a0f1e] border border-white/5 rounded-2xl p-4 animate-pulse h-24" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard
          label="Total Orders" accent
          value={stats?.total_orders ?? 0}
          sub={`${stats?.pending_orders ?? 0} pending`}
          icon="📦" to="/admin/orders"
        />
        <StatCard
          label="Total Revenue"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        <Link to="/admin/messages" className="flex items-center gap-3 bg-[#0a0f1e] border border-white/5 rounded-xl p-3 hover:border-white/10 transition-all">
          <span className="text-xl">✉️</span>
          <div>
            <p className="text-white text-sm font-bold">{stats?.unread_messages ?? 0} unread</p>
            <p className="text-gray-500 text-xs">Contact messages</p>
          </div>
        </Link>
        <Link to="/admin/reviews" className="flex items-center gap-3 bg-[#0a0f1e] border border-white/5 rounded-xl p-3 hover:border-white/10 transition-all">
          <span className="text-xl">⭐</span>
          <div>
            <p className="text-white text-sm font-bold">{stats?.pending_reviews ?? 0} pending</p>
            <p className="text-gray-500 text-xs">Reviews to approve</p>
          </div>
        </Link>
        <Link to="/admin/restaurant" className="flex items-center gap-3 bg-[#0a0f1e] border border-white/5 rounded-xl p-3 hover:border-white/10 transition-all">
          <span className="text-xl">⚙️</span>
          <div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Link to="/admin/orders" className="text-amber-400 text-xs hover:text-amber-300 transition-colors">
            View all →
          </Link>
        </div>
              ...
              {recentOrders.map((order) => (
                <div key={order.order_number} className="flex items-center justify-between px-4 py-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="text-amber-400 font-mono text-xs shrink-0">#{order.order_number}</span>
                  ...
                  <div className="flex items-center gap-3 shrink-0 ml-3">
                    <span className="text-white text-xs font-semibold">€{Number(order.total).toFixed(2)}</span>
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium border capitalize ${STATUS_STYLES[order.status] ?? STATUS_STYLES.pending}`}>
