...
export default function OrderConfirmedPage() {
  ...
  const { orderNumber, items, orderType, address, customerName, total, subtotal, deliveryCharge, discountAmount } = state;
    ...
    <main className="bg-gray-950 min-h-screen pt-16">
      <div className="max-w-xl mx-auto px-4 py-12">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">{t('Tilaus Vahvistettu!', 'Order Confirmed!')}</h1>
          <p className="text-gray-400">{t('Kiitos tilauksestasi,', 'Thank you for your order,')} {customerName}!</p>
        ...
        <div className="bg-gray-900 border border-white/5 rounded-xl p-5 mb-5">
          <p className="text-gray-400 text-sm mb-1">{t('Arvioitu toimitusaika', 'Estimated Delivery')}</p>
          <p className="text-2xl font-bold text-amber-400">30-45 {t('minuuttia', 'minutes')}</p>
        </div>

        <div className="bg-gray-900 border border-white/5 rounded-xl p-5 mb-5">
          <h2 className="text-white font-bold text-sm mb-3">{t('Tilatut Tuotteet', 'Ordered Items')}</h2>
            ...
            {items.map((ci, i) => (
              <div key={i} className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-gray-500 text-xs">{t('Koko', 'Size')}: {ci.size} {ci.toppings.length > 0 && `+ ${ci.toppings.length} ${t('lisäkettä', 'toppings')}`}</p>
                </div>
          ...
          <div className="border-t border-white/10 pt-3 space-y-1 text-sm">
            ...
            {orderType === 'delivery' && <div className="flex justify-between text-gray-400"><span>{t('Toimitus', 'Delivery')}</span><span>€{deliveryCharge.toFixed(2)}</span></div>}
            ...
            <div className="flex justify-between text-white font-bold pt-2 border-t border-white/10"><span>{t('Yhteensä', 'Total')}</span><span>€{total.toFixed(2)}</span></div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Link to="/account" className="flex-1 flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-400 text-gray-900 font-bold py-3 rounded-xl transition-colors">
            <User size={18} />{t('Oma Tili', 'My Account')}<ArrowRight size={16} />
          </Link>
          <Link to="/" className="flex-1 flex items-center justify-center gap-2 bg-gray-900 hover:bg-gray-800 border border-white/10 text-white font-semibold py-3 rounded-xl transition-colors">
            <Home size={18} />{t('Etusivulle', 'Back to Home')}
          </Link>
        </div>
      </div>
    </main>
